...
const firebaseConfig = {
  apiKey: (process.env.NEXT_PUBLIC_FIREBASE_API_KEY || '<YOUR_API_KEY>').trim(),
  authDomain: (process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN || '<YOUR_AUTH_DOMAIN>').trim(),
  projectId: (process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || '<YOUR_PROJECT_ID>').trim(),
  storageBucket: (process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET || '<YOUR_STORAGE_BUCKET>').trim(),
  messagingSenderId: (
    process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID || '<YOUR_MESSAGING_SENDER_ID>'
  ).trim(),
  appId: (process.env.NEXT_PUBLIC_FIREBASE_APP_ID || '<YOUR_APP_ID>').trim(),
};
