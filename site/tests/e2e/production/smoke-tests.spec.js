...
test.describe('Production Smoke Tests - Firebase Health', () => {
  test('Firebase SDK initializes without errors', async ({ page }) => {
    const consoleErrors = [];
    page.on('console', (msg) => {
  ...
  test('Firestore is NOT in offline mode', async ({ page }) => {
    ...
    const offlineIndicators = consoleMessages.filter(
      (msg) =>
    ...
    expect(offlineIndicators.length).toBe(0);

    console.log('✅ Firestore is online (no offline mode detected)');
  });
...
test.describe('Production Smoke Tests - Console Health', () => {
  ...
  test('error count stays below threshold', async ({ page }) => {
    const allMessages = {
      errors: 0,
      warnings: 0,
      logs: 0,
    ...
    page.on('console', (msg) => {
      if (msg.type() === 'error') allMessages.errors++;
      if (msg.type() === 'warning') allMessages.warnings++;
      if (msg.type() === 'log') allMessages.logs++;
    });
    ...
    // Error threshold: <5 errors per page
    expect(allMessages.errors).toBeLessThan(5);

    console.log(
      `✅ Console health: ${allMessages.errors} errors, ${allMessages.warnings} warnings`
    );
  });
});
